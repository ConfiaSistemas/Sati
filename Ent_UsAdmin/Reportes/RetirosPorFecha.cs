...
namespace ConfiaAdmin
{
    ...
    public partial class RetirosPorFecha
    {
        ...
        private void BackgroundWorker1_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            ...
            {
                if (Conversion.Val(row.Cells[8].Value) == 0d)
                {
                    total = Conversions.ToDouble(Operators.AddObject(total, row.Cells[1].Value));
                }
                else
                {
                    total = Conversions.ToDouble(Operators.AddObject(total, row.Cells[8].Value));
                }
