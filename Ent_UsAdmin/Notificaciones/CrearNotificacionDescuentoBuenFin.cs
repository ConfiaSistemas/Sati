...
namespace ConfiaAdmin
{
    ...
    public partial class CrearNotificacionDescuentoBuenFin
    {
        ...
        public CrearNotificacionDescuentoBuenFin()
        {
            InitializeComponent();
        }
        ...
        private void BackgroundNotificacion_DoWork(object sender, DoWorkEventArgs e)
        {
            try
            {
                ...
                consultaLogin = "insert into Notificaciones values(null,'DescuentoBuenFin',1,'" + Module1.nmusr + "','" + ComboUsuarios.selectedValue + "','" + idPromesa + "','','" + txtComentario.Text + "',0,'0','" + DateTime.Now.ToString("yyyy-MM-dd") + "','" + DateTime.Now.ToString("HH:mm:ss") + "',0,'','','" + Module1.nombreAmostrar + "','',''); ";
        ...
        private void CrearNotificacionDescuentoPromesa_FormClosing(object sender, FormClosingEventArgs e)
        {
            if (creada)
            {
                if (tipoCredito == "L")
                {
                    My.MyProject.Forms.PromPagoLegalBuenFin.creada = true;
                }
                else
                {
                    My.MyProject.Forms.PromPagoBuenFin.creada = true;
                }
            }

            else if (tipoCredito == "L")
            {
                My.MyProject.Forms.PromPagoLegalBuenFin.creada = false;
            }
            else
            {
                My.MyProject.Forms.PromPagoBuenFin.creada = false;
