...
Public Class InformacionEmpeño
    ...
    Private Sub BackgroundClientes_DoWork(sender As Object, e As System.ComponentModel.DoWorkEventArgs) Handles BackgroundClientes.DoWork
        ...
        consultaCredito = "select format(FechaEntrega,'dd-MM-yy') as FechaEntrega,credito.Nombre,Monto,Credito.Pagare,TiposDeCredito.Nombre as tipo,credito.estado from Credito inner join TiposDeCredito on Credito.Tipo = TiposDeCredito.id where credito.id = '" & idEmpeño & "'"
        ...
        consultaClientes = "select Clientes.id,Clientes.Nombre,Clientes.Apellidopaterno,Clientes.ApellidoMaterno from Credito inner join Solicitud on Credito.IdSolicitud =  Solicitud.id inner join DatosSolicitud on Solicitud.id = DatosSolicitud.IdSolicitud inner join Clientes on DatosSolicitud.IdCliente = Clientes.id where Credito.id = '" & idEmpeño & "'"
    ...
    Private Sub BackgroundSolicitud_DoWork(sender As Object, e As DoWorkEventArgs) Handles BackgroundSolicitud.DoWork
        ...
        Try
            ...
            strimpuestos = "select Solicitud.id,format(Solicitud.Fecha,'dd-MM-yy') as Fecha,Solicitud.Monto,Solicitud.MontoAutorizado,Solicitud.Tipo from Credito inner join Solicitud on Credito.IdSolicitud = Solicitud.id where Credito.id = '" & idEmpeño & "'"
    ...
    Private Sub BackgroundCalendario_DoWork(sender As Object, e As DoWorkEventArgs) Handles BackgroundCalendario.DoWork
        ...
        consultaCalendario = "select format(CalendarioNormal.FechaPago,'dd-MM-yy') as FechaPago,CalendarioNormal.Npago,CalendarioNormal.Monto from Credito inner join CalendarioNormal on Credito.id = CalendarioNormal.id_credito where Credito.id = '" & idEmpeño & "'"
    ...
    Private Sub BackgroundDocumentos_DoWork(sender As Object, e As DoWorkEventArgs) Handles BackgroundDocumentos.DoWork
        ...
        consultaDocumentos = "select TiposdeDocumentosSolicitud.Nombre,DocumentosCredito.imagen from Credito inner join DocumentosCredito on Credito.id = DocumentosCredito.idEmpeño inner join TiposdeDocumentosSolicitud on DocumentosCredito.Tipo = TiposdeDocumentosSolicitud.id where Credito.id = '" & idEmpeño & "'"
    ...
    Private Sub BackgroundPagos_DoWork(sender As Object, e As DoWorkEventArgs) Handles BackgroundPagos.DoWork
        ...
        ' Else ISNULL((Select nombre from Credito where id = pagos.idEmpeño),0) 		End) As nombre,idEmpeño,Fecha,Hora,PagoNormal,Intereses,Total,tipo,Caja from
        '(select Ticket.Id,Ticket.Recibido,Ticket.idEmpeño,Ticket.Fecha,Ticket.hora,Ticket.PagoNormal,Ticket.Intereses,Ticket.total,tipodoc.Nombre as tipo,Ticket.concepto,Ticket.caja from Ticket  inner join TipoDoc on Ticket.tipodoc = TipoDoc.id  where  ticket.idEmpeño = '" & idEmpeño & "'  )pagos order by Fecha,Hora asc"

        consulta = "if  exists(select * from ConveniosSac where idEmpeño = '" & idEmpeño & "')
begin
select id,Recibido,(case when tipo = 'Legal' then Concepto
							   when tipo = 'Extra' then Concepto
							  
							   else ISNULL((select nombre from Credito where id = pagos.idEmpeño),0) 		end) as nombre,idEmpeño,Fecha,Hora,PagoNormal,Intereses,Total,tipo,Caja from
(select Ticket.Id,Ticket.Recibido,Ticket.idEmpeño,Ticket.Fecha,Ticket.hora,Ticket.PagoNormal,Ticket.Intereses,Ticket.total,tipodoc.Nombre as tipo,Ticket.concepto,Ticket.caja from Ticket  inner join TipoDoc on Ticket.tipodoc = TipoDoc.id  where  ticket.idEmpeño = '" & idEmpeño & "' and (tipodoc = (select id from tipodoc where nombre = 'Pago') or  tipodoc = (select id from tipodoc where nombre = 'Liquidación Insoluto') or tipodoc = (select id from tipodoc where nombre = 'Liquidación Renovación') or tipodoc = (select id from tipodoc where nombre = 'Liquidación Normal') or  tipodoc = (select id from tipodoc where nombre = 'Renovación Insoluto')) )pagos 
union
select id,Recibido,(case when tipo = 'Legal' then Concepto
							   when tipo = 'Extra' then Concepto
							  
							   else ISNULL((select nombre from Credito inner join ConveniosSac on Credito.id = ConveniosSac.idEmpeño where ConveniosSac.id = pagos.idEmpeño),0) 		end) as nombre,idEmpeño,Fecha,Hora,PagoNormal,Intereses,Total,tipo,Caja from
(select Ticket.Id,Ticket.Recibido,Ticket.idEmpeño,Ticket.Fecha,Ticket.hora,Ticket.PagoNormal,Ticket.Intereses,Ticket.total,tipodoc.Nombre as tipo,Ticket.concepto,Ticket.caja from Ticket  inner join TipoDoc on Ticket.tipodoc = TipoDoc.id  where  ticket.idEmpeño = (select id from ConveniosSac where idEmpeño ='" & idEmpeño & "') and TipoDoc = (select id from tipodoc where nombre = 'Convenio') )pagos order by Fecha,Hora asc
end
else if not exists(select * from ConveniosSac where idEmpeño = '" & idEmpeño & "')
begin
select id,Recibido,(case when tipo = 'Legal' then Concepto
							   when tipo = 'Extra' then Concepto
...
"
    ...
    Private Sub btnGenerarCalendario_Click(sender As Object, e As EventArgs) Handles btnGenerarCalendario.Click
        'DocumentosCredito.idEmpeño = idEmpeño
        'DocumentosCredito.Show()
        'Dijo Jairo que esto lo podriamos reutilizar
    End Sub
    ...
    Private Sub BackgroundActualizaciones_DoWork(sender As Object, e As DoWorkEventArgs) Handles BackgroundActualizaciones.DoWork
        ...
        consultaActualizaciones = "select Fecha,Hora,Campo,ValorAnterior,NuevoValor,Motivo from actualizacionescredito where idEmpeño = '" & idEmpeño & "'"
    ...
    Private Sub BackgroundComportamiento_DoWork(sender As Object, e As DoWorkEventArgs) Handles BackgroundComportamiento.DoWork
        ...
        consulta = "if  exists(select * from ConveniosSac where idEmpeño = '" & idEmpeño & "')
begin
select 'Normal' as TipoDePago,idPago,Npago,Monto,Interes,Abonado,Pendiente,FechaPago,FechaUltimoPago from CalendarioNormal where id_credito = '" & idEmpeño & "' and Estado = 'L'
union
select 'Creación de convenio' as TipoDePago,'','',DeudaCredito,Moratorios,'',TotalDeuda,Fecha,'' from ConveniosSac where idEmpeño = '" & idEmpeño & "'
union
select 'Convenio' as TipoDePago,idPago,Npago,Monto,Interes,Abonado,Pendiente,FechaPago,Fecha from CalendarioConveniosSac where idConvenio = (select id from ConveniosSac where idEmpeño = '" & idEmpeño & "') order by FechaPago asc
end
else if not exists(select * from ConveniosSac where idEmpeño = '" & idEmpeño & "')
begin
select 'Normal' as TipoDePago,idPago,Npago,Monto,Interes,Abonado,Pendiente,FechaPago,FechaUltimoPago from CalendarioNormal where id_credito = '" & idEmpeño & "' 
end
"
            ...
            While readerTicket.Read
                ...
                Select Case readerTicket("TipoDePago")
                    ...
                    Case "Creación de convenio"
                        ...
                        Dim consultaDetalle As String
                        Dim readerDetalle As SqlDataReader
                        consultaDetalle = "select idPago,Npago,Monto,Interes,Abonado,Pendiente,FechaPago,FechaUltimoPago from CalendarioNormal where id_credito = '" & idEmpeño & "' and (Estado = 'V' or Estado = 'P')"
                        COMANDOdetalle = New SqlCommand
                        COMANDOdetalle.Connection = conexionempresa
                        COMANDOdetalle.CommandText = consultaDetalle
                        readerDetalle = COMANDOdetalle.ExecuteReader
