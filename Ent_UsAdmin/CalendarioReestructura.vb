...
Public Class CalendarioReestructura
    ...
    Private Sub BackgroundConvenio_DoWork(sender As Object, e As System.ComponentModel.DoWorkEventArgs) Handles BackgroundConvenio.DoWork
        ...
        consultaConvenio = "insert into ReestructurasSac values('" & idCredito & "','" & cantPagos & "','" & Moratorios & "','" & Capital & "','" & deudaTotal & "','" & pagoprimero & "','" & ultimopago & "','" & Date.Now.ToString("yyyy-MM-dd") & "','" & gestor & "','A') select SCOPE_IDENTITY()"
        ...
        For Each row As DataGridViewRow In dtimpuestos.Rows
            ...
            consultaCalendario = "Insert into calendarioReestructurasSac values('" & numero & "','" & row.Cells(2).Value & "','" & row.Cells(1).Value & "','0','" & row.Cells(0).Value & "','','" & idConvenio & "','P','" & row.Cells(3).Value & "','" & row.Cells(3).Value & "','0')"
