...
Public Class CrearNotificacionDescuentoBuenFin
    ...
    Private Sub BackgroundNotificacion_DoWork(sender As Object, e As DoWorkEventArgs) Handles BackgroundNotificacion.DoWork
        Try
            ...
            consultaLogin = "insert into Notificaciones values(null,'DescuentoBuenFin',1,'" & nmusr & "','" & ComboUsuarios.selectedValue & "','" & idPromesa & "','','" & txtComentario.Text & "',0,'0','" & Date.Now.ToString("yyyy-MM-dd") & "','" & Date.Now.ToString("HH:mm:ss") & "',0,'','','" & nombreAmostrar & "','',''); "
    ...
    Private Sub CrearNotificacionDescuentoPromesa_FormClosing(sender As Object, e As FormClosingEventArgs) Handles Me.FormClosing
        If creada Then
            If tipoCredito = "L" Then
                PromPagoLegalBuenFin.creada = True
            Else
                PromPagoBuenFin.creada = True
            End If

        Else
            If tipoCredito = "L" Then
                PromPagoLegalBuenFin.creada = False
            Else
                PromPagoBuenFin.creada = False
            End If
