...
Public Class SolicitudPorFecha
...
(select * from Empleados where Tipo = 'P') Promotores on cred.IdPromotor = Promotores.id) desembolsos"
            ...
            Case "Promotor"
                If ComboElección.Text = "Todos" Then
                    ...
                    consulta = "select Fecha,id,Nombre,Monto,Tipo,Gestor,Promotor,case when CanCreditos > 1 then 'Renovación' else 'Nuevo' end as 'Nuevo o Renovación',Estado from
(select Fecha,cred.id,cred.Nombre,format(cred.Monto,'C','es-mx') as Monto,cred.Tipo,Gestores.Nombre as Gestor,Promotores.Nombre as Promotor,isnull((select COUNT(Solicitud.id) from Solicitud inner join DatosSolicitud on DatosSolicitud.IdSolicitud = Solicitud.id where DatosSolicitud.IdCliente = cred.IdCliente ),0) as CanCreditos,Estado from
(select Fecha,Solicitud.id,Solicitud.Nombre,Monto,TiposDeCredito.Nombre as tipo,IdPromotor,IdGestor,IdCliente,Estado from Solicitud inner join TiposDeCredito on Solicitud.Tipo = TiposDeCredito.id  where  (Fecha >= '" & Format(dateDesde.Value, "yyyy-MM-dd") & "' and Fecha <= '" & Format(dateHasta.Value, "yyyy-MM-dd") & "')) cred inner join
(select * from Empleados where Tipo = 'G') Gestores on cred.IdGestor = Gestores.id inner join
(select * from Empleados where Tipo = 'P') Promotores on cred.IdPromotor = Promotores.id) desembolsos"
                Else
                    '                    consulta = "select FechaEntrega,cred.id,cred.Nombre,format(cred.Monto,'C','es-mx') as Monto,format(cred.Pagare,'C','es-mx') as Pagaré,cred.Tipo,Promotores.Nombre as Promotor from
...
(select * from Empleados where Tipo = 'P') Promotores on cred.IdPromotor = Promotores.id) desembolsos"
    ...
    Private Sub BackgroundConsulta_RunWorkerCompleted(sender As Object, e As RunWorkerCompletedEventArgs) Handles BackgroundConsulta.RunWorkerCompleted
        ...
        Dim totalIncompletas As Double = 0
        Dim totalEspera As Double = 0
        Dim totalVerificadas As Double = 0
        Dim totalAprobadas As Double = 0
        Dim totalRechazadas As Double = 0
        For Each row As DataGridViewRow In dtimpuestos.Rows
            ...
            ElseIf row.Cells("Estado").Value = "E" Then
                totalEspera += 1
            ElseIf row.Cells("Estado").Value = "V" Then
                totalVerificadas += 1

            ElseIf row.Cells("Estado").Value = "A" Then
                totalAprobadas += 1

            ElseIf row.Cells("Estado").Value = "R" Then
                totalRechazadas += 1
        ...
        lblIncompletas.Text = totalIncompletas
        lblEspera.Text = totalEspera
        lblVerificadas.Text = totalVerificadas
        lblAprobadas.Text = totalAprobadas
        lblRechazadas.Text = totalRechazadas
        Cargando.Close()
