...
namespace ConfiaAdmin
{
    ...
    public partial class AplicarDescuentoBuenFin
    {
        ...
        public AplicarDescuentoBuenFin()
        {
            InitializeComponent();
        }
        ...
        {
            My.MyProject.Forms.AutorizacionNotificacionBuenFin.tipoAutorizacion = "SatiModCreditosDescuentoPromesa";
            My.MyProject.Forms.AutorizacionNotificacionBuenFin.ShowDialog();
            if (Autorizado)
            {
            ...
            if (estadoNotificacion == "A")
            {
                if (conectado)
                {
                    ...
                    foreach (DataGridViewRow row in dtDetalle.Rows)
                    {
                        ...
                        SqlCommand ComandoAplicarDescuento;
                        string consultaAplicarDescuento;
                        consultaAplicarDescuento = "Quitarmulta";
                        ComandoAplicarDescuento = new SqlCommand();
                        ComandoAplicarDescuento.Connection = conex;
                        ComandoAplicarDescuento.CommandText = consultaAplicarDescuento;
                        ComandoAplicarDescuento.CommandType = CommandType.StoredProcedure;
                        ComandoAplicarDescuento.Parameters.AddWithValue("Dias", "");
                        ComandoAplicarDescuento.Parameters.AddWithValue("Idpago", "");
                        ComandoAplicarDescuento.Parameters.AddWithValue("Idc", row.Cells[0].Value);
                        if (Conversions.ToBoolean(Operators.ConditionalCompareObjectEqual(row.Cells[12].Value, "L", false)))
                        {
                            ComandoAplicarDescuento.Parameters.AddWithValue("tipo", "100%");
                        }
                        else
                        {
                            ComandoAplicarDescuento.Parameters.AddWithValue("tipo", "90%");
                        }

                        ComandoAplicarDescuento.Parameters.AddWithValue("Motivo", "Descuento Buen Fin");
                        ComandoAplicarDescuento.Parameters.AddWithValue("Usuario", Module1.nmusr);
                        if (Conversions.ToBoolean(Operators.ConditionalCompareObjectEqual(row.Cells[12].Value, "L", false)))
                        {
                            ComandoAplicarDescuento.Parameters.AddWithValue("tipocredito", "Legal");
                        }
                        else
                        {
                            ComandoAplicarDescuento.Parameters.AddWithValue("tipocredito", "Normal");

                        }

                        ComandoAplicarDescuento.ExecuteNonQuery();
