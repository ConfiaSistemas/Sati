...
namespace ConfiaAdmin
{
    ...
    public partial class DocumentosCredito
    {
        public string idCredito;
        public string frmNombre;

        public DocumentosCredito()
        {
            InitializeComponent();
        }
        private void btn_agregar_Click(object sender, EventArgs e)
        {
            My.MyProject.Forms.AgregarDocumentoCredito.ShowDialog();
        }
        ...
        private void BackgroundDocumentos_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            foreach (DataGridViewRow row in dtdatosDocumentosNuevos.Rows)
            {
                ...
                consultaDocumento = Conversions.ToString(Operators.ConcatenateObject(Operators.ConcatenateObject("insert into DocumentosCredito values('" + idCredito + "','", row.Cells[0].Value), "',@Documento)"));
        ...
        private void BackgroundDocumentos_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            foreach (Form frm in Application.OpenForms)
            {
                if ((frm.Text ?? "") == (frmNombre ?? ""))
                {
                    ...
                    Invoke(new Action(() => frmn.BackgroundDocumentos.RunWorkerAsync()));
                }
            }
