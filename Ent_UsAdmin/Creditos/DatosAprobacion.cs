...
namespace ConfiaAdmin
{
    ...
    public partial class DatosAprobacion
    {
        ...
        private void BackgroundWorker1_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            consultaDatosSolicitud = "select datosSolicitud.idSolicitud ,datosSolicitud.id,datosSolicitud.idCliente,(clientes.nombre +' '+clientes.ApellidoPaterno+' '+clientes.ApellidoMaterno) as nombrecompleto,clientes.nombre,clientes.ApellidoPaterno,clientes.ApellidoMaterno,clientes.FechaNacimiento,clientes.Telefono,clientes.Celular,datosSolicitud.estado,datosSolicitud.MontoAutorizado from datosSolicitud inner join clientes on datosSolicitud.idCliente = clientes.id where datosSolicitud.idSolicitud = '" + idSolicitud + "'";
            ...
            while (readerSolicitud.Read())
            {
                ...
                moto = Conversions.ToBoolean(readerSolicitud["Moto"]);
                Enganche = Conversions.ToDouble(readerSolicitud["Enganche"]);
            }
        ...
        private void ConsultarCliente(string idCliente)
        {
            ...
            foreach (DataRow row in dataDatos.Rows)
            {
                if ((row["idCliente"].ToString() ?? "") == (idCliente ?? ""))
                {
                    ...
                    txtMontoVerificacion.Text = Conversions.ToString(row["MontoVerificacion"]);
                    txtMontoAutorizado.Text = Conversions.ToString(row["MontoVerificacion"]);
        ...
        private void BackgroundAct_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            switch (estadoDatosSolicitud ?? "")
            {
                ...
                case "I":
                    {
                        consultaActDatos = "update DatosSolicitud set Comentarios = '" + txtComentarios.Text + "',estado = '" + estadoDatosSolicitud + "' where id = '" + idDatosSolicitud + "'";
                        break;
                    }
        ...
        private void ActDatosTabla()
        {
            ...
            string consultaDatosSolicitud;
            SqlDataReader readerDatosSolicitud;
            consultaDatosSolicitud = "select datosSolicitud.id ,datosSolicitud.idSolicitud ,datosSolicitud.idCliente,(clientes.nombre +' '+clientes.ApellidoPaterno+' '+clientes.ApellidoMaterno) as nombrecompleto,clientes.nombre,clientes.ApellidoPaterno,clientes.ApellidoMaterno,clientes.FechaNacimiento,clientes.Telefono,clientes.Celular,datosSolicitud.estado,datosSolicitud.MontoAutorizado from datosSolicitud inner join clientes on datosSolicitud.idCliente = clientes.id where datosSolicitud.idSolicitud = '" + idSolicitud + "'";
            comandoDatosSolicitud = new SqlCommand();
            comandoDatosSolicitud.Connection = Module1.conexionempresa;
            comandoDatosSolicitud.CommandText = consultaDatosSolicitud;
            readerDatosSolicitud = comandoDatosSolicitud.ExecuteReader();
        ...
        private void btn_a_verificacion_Click(object sender, EventArgs e)
        {
            ...
            My.MyProject.Forms.Autorizacion.tipoAutorizacion = "SatiModSolicitudesAprobar";
        ...
        private void BackgroundVerificacion_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            double montoAutorizadoMotocicleta =0;

            if (moto)
            {
                montoAutorizadoMotocicleta = montoAutorizadoTotal - montoAutorizadoTotal * (Enganche / 100d);
            }
                ...
                case "S":
                    {
                        if (moto)
                        {
                            pagoIndividualCredito = montoAutorizadoMotocicleta / 1000d * InteresSolicitud;
                        }
                        else
                        {
                ...
                case "Q":
                    {
                        if (moto)
                        {
                            pagoIndividualCredito = montoAutorizadoMotocicleta / 1000d * InteresSolicitud * 2d;
                        }
                        else
                        {
            ...
            if (moto)
            {
                consultaCredito = "insert into Credito(Fecha,Hora,Nombre,Monto,Pagare,Tipo,Integrantes,PagoIndividual,Plazo,Interes,IdCliente,IdPromotor,IdGestor,IdSolicitud,Estado) values('" + DateTime.Now.ToString("yyyy-MM-dd") + "','" + tiempo + "','" + nombreSolicitud + "','" + montoAutorizadoMotocicleta + "','" + PagareCredito + "','" + TipoSolicitud + "','" + Aprobados + "','" + pagoIndividualCredito + "','" + PlazoSolicitud + "','" + InteresSolicitud + "','" + idClienteSolicitud + "','" + idPromotorSolicitud + "','" + idGestorSolicitud + "','" + idSolicitud + "','E') select SCOPE_IDENTITY()";
            }
     ...
     //       if (RadioRechazado.Checked)
      ...
      //      }
        ...
        private void BackgroundIncompleta_DoWork(object sender, DoWorkEventArgs e)
        {
            Module1.iniciarconexionempresa();
            string tiempo = DateAndTime.TimeOfDay.ToString("HH:mm:ss");
            SqlCommand comandoActEstado;
            string consultaActEstado;
            consultaActEstado = "update Solicitud set estado = 'I',autorizadoPor='" + Module1.nmusr + "' where id = '" + idSolicitud + "'";
            comandoActEstado = new SqlCommand();
            comandoActEstado.Connection = Module1.conexionempresa;
            comandoActEstado.CommandText = consultaActEstado;
            comandoActEstado.ExecuteNonQuery();
        ...
        {
            estadoDatosSolicitud = "I";
            My.MyProject.Forms.Autorizacion.tipoAutorizacion = "SatiModSolicitudesAprobar";
            My.MyProject.Forms.Autorizacion.ShowDialog();
            if (autorizado)
            {
                My.MyProject.Forms.Cargando.Show();
                My.MyProject.Forms.Cargando.MonoFlat_Label1.Text = "Actualizando Solicitud";
                backgroundIncompleta.RunWorkerAsync();
            }
            else
            {
                MessageBox.Show("No fue autorizado");
        ...
        private void backgroundIncompleta_DoWork_1(object sender, DoWorkEventArgs e)
        {
