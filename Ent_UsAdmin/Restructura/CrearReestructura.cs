...
namespace ConfiaAdmin
{
    ...
    public partial class CrearReestructura
    {
...
(select * from Empleados where Tipo = 'P') Promotores on Cartera.IdPromotor = Promotores.id ) CarteraTotal order by nombre asc";
        ...
        private void btnGenerarCalendario_Click(object sender, EventArgs e)
        {
            ...
            if (Module1.idGrp == 1)
            {
                My.MyProject.Forms.CalendarioReestructura.Moratorios = ParteMoratorios;
                My.MyProject.Forms.CalendarioReestructura.Capital = ParteCredito;
                My.MyProject.Forms.CalendarioReestructura.idCredito = Conversions.ToInteger(idCredito);
                My.MyProject.Forms.CalendarioReestructura.deuda = DeudaTotal;
                My.MyProject.Forms.CalendarioReestructura.cantPagos = Conversions.ToInteger(txtCantPagos.Text);
                My.MyProject.Forms.CalendarioReestructura.MontoPago = Conversions.ToDouble(txtPago.Text);
                My.MyProject.Forms.CalendarioReestructura.PrimerPago = Convert.ToDateTime(datePrimerPago.Value.ToShortDateString());
                My.MyProject.Forms.CalendarioReestructura.Modalidad = Modalidad;
                My.MyProject.Forms.CalendarioReestructura.deudaTotal = DeudaTotal;
                My.MyProject.Forms.CalendarioReestructura.gestor = gestor;
                My.MyProject.Forms.CalendarioReestructura.Show();
            }

            else if (diasCancelado < 30 | abonadoCancelado < 1000d)
            {
                MessageBox.Show("Para crear la reestructura, deben haber pasado por lo menos 30 días desde la cancelación del convenio, y tener abonados por lo menos $1,000.00 desde esa fecha.");
            }
            else
            {
                My.MyProject.Forms.CalendarioReestructura.Moratorios = ParteMoratorios;
