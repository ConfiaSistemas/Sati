...
namespace ConfiaAdmin
{
    ...
    public partial class SolicitudPorFecha
    {
...
(select FechaEntrega,Credito.id,credito.Nombre,Monto,Pagare,TiposDeCredito.Nombre as tipo,IdPromotor,IdGestor,IdCliente,Estado from Credito inner join TiposDeCredito on Credito.Tipo = TiposDeCredito.id  where FechaEntrega != null and (FechaEntrega >= '" + Strings.Format(dateDesde.Value, "yyyy-MM-dd") + "' and FechaEntrega <= '" + Strings.Format(dateHasta.Value, "yyyy-MM-dd") + @"')) cred inner join
...
(select * from Empleados where Tipo = 'P') Promotores on cred.IdPromotor = Promotores.id) desembolsos";
                        ...
                        else
                        {
                            ...
                            // (select * from Empleados where Tipo = 'G') Gestores on cred.IdGestor = Gestores.id
                            // "
                            consulta = @"select Fecha,id,Nombre,Monto,Tipo,Gestor,Promotor,case when CanCreditos > 1 then 'Renovación' else 'Nuevo' end as 'Nuevo o Renovación',Estado from
(select Fecha,cred.id,cred.Nombre,format(cred.Monto,'C','es-mx') as Monto,cred.Tipo,Gestores.Nombre as Gestor,Promotores.Nombre as Promotor,isnull((select COUNT(Solicitud.id) from Solicitud inner join DatosSolicitud on DatosSolicitud.IdSolicitud = Solicitud.id where DatosSolicitud.IdCliente = cred.IdCliente ),0) as CanCreditos,Estado from
(select Fecha,Solicitud.id,Solicitud.Nombre,Monto,TiposDeCredito.Nombre as tipo,IdPromotor,IdGestor,IdCliente,Estado from Solicitud inner join TiposDeCredito on Solicitud.Tipo = TiposDeCredito.id  where solicitud.idgestor = '" + idEmpleado + "' and (Fecha >= '" + Strings.Format(dateDesde.Value, "yyyy-MM-dd") + "' and Fecha <= '" + Strings.Format(dateHasta.Value, "yyyy-MM-dd") + @"')) cred inner join
(select * from Empleados where Tipo = 'G') Gestores on cred.IdGestor = Gestores.id inner join
(select * from Empleados where Tipo = 'P') Promotores on cred.IdPromotor = Promotores.id) desembolsos";
                        }
                ...
                case "Promotor":
                    {
                        if (ComboElección.Text == "Todos")
                        {
                            ...
                            consulta = @"select Fecha,id,Nombre,Monto,Tipo,Gestor,Promotor,case when CanCreditos > 1 then 'Renovación' else 'Nuevo' end as 'Nuevo o Renovación',Estado from
(select Fecha,cred.id,cred.Nombre,format(cred.Monto,'C','es-mx') as Monto,cred.Tipo,Gestores.Nombre as Gestor,Promotores.Nombre as Promotor,isnull((select COUNT(Solicitud.id) from Solicitud inner join DatosSolicitud on DatosSolicitud.IdSolicitud = Solicitud.id where DatosSolicitud.IdCliente = cred.IdCliente ),0) as CanCreditos,Estado from
(select Fecha,Solicitud.id,Solicitud.Nombre,Monto,TiposDeCredito.Nombre as tipo,IdPromotor,IdGestor,IdCliente,Estado from Solicitud inner join TiposDeCredito on Solicitud.Tipo = TiposDeCredito.id  where  (Fecha >= '" + Strings.Format(dateDesde.Value, "yyyy-MM-dd") + "' and Fecha <= '" + Strings.Format(dateHasta.Value, "yyyy-MM-dd") + @"')) cred inner join
(select * from Empleados where Tipo = 'G') Gestores on cred.IdGestor = Gestores.id inner join
(select * from Empleados where Tipo = 'P') Promotores on cred.IdPromotor = Promotores.id) desembolsos";
                        }
                        else
                        {
                            ...
                            consulta = @"select Fecha,id,Nombre,Monto,Tipo,Gestor,Promotor,case when CanCreditos > 1 then 'Renovación' else 'Nuevo' end as 'Nuevo o Renovación',Estado from
(select Fecha,cred.id,cred.Nombre,format(cred.Monto,'C','es-mx') as Monto,cred.Tipo,Gestores.Nombre as Gestor,Promotores.Nombre as Promotor,isnull((select COUNT(Solicitud.id) from Solicitud inner join DatosSolicitud on DatosSolicitud.IdSolicitud = Solicitud.id where DatosSolicitud.IdCliente = cred.IdCliente ),0) as CanCreditos,Estado from
(select Fecha,Solicitud.id,Solicitud.Nombre,Monto,TiposDeCredito.Nombre as tipo,IdPromotor,IdGestor,IdCliente,Estado from Solicitud inner join TiposDeCredito on Solicitud.Tipo = TiposDeCredito.id  where solicitud.idpromotor = '" + idEmpleado + "' and (Fecha >= '" + Strings.Format(dateDesde.Value, "yyyy-MM-dd") + "' and Fecha <= '" + Strings.Format(dateHasta.Value, "yyyy-MM-dd") + @"')) cred inner join
(select * from Empleados where Tipo = 'G') Gestores on cred.IdGestor = Gestores.id inner join
(select * from Empleados where Tipo = 'P') Promotores on cred.IdPromotor = Promotores.id) desembolsos";
                        }
...
(select * from Empleados where Tipo = 'P') Promotores on cred.IdPromotor = Promotores.id) desembolsos";
        ...
        private void BackgroundConsulta_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            ...
            double totalIncompletas = 0d;
            double totalEspera = 0d;
            double totalVerificadas = 0d;
            double totalAprobadas = 0d;
            double totalRechazadas = 0d;
            foreach (DataGridViewRow row in dtimpuestos.Rows)
            {
                ...
                if (Conversions.ToBoolean(Operators.ConditionalCompareObjectEqual(row.Cells["Estado"].Value, "I", false)))
                {
                    totalIncompletas += 1d;
                }


                else if (Conversions.ToBoolean(Operators.ConditionalCompareObjectEqual(row.Cells["Estado"].Value, "E", false)))
                {
                    totalEspera += 1d;
                }
                else if (Conversions.ToBoolean(Operators.ConditionalCompareObjectEqual(row.Cells["Estado"].Value, "V", false)))
                {
                    totalVerificadas += 1d;
                }

                else if (Conversions.ToBoolean(Operators.ConditionalCompareObjectEqual(row.Cells["Estado"].Value, "A", false)))
                {
                    totalAprobadas += 1d;
                }

                else if (Conversions.ToBoolean(Operators.ConditionalCompareObjectEqual(row.Cells["Estado"].Value, "R", false)))
                {
                    totalRechazadas += 1d;
            ...
            lblIncompletas.Text = totalIncompletas.ToString();
            lblEspera.Text = totalEspera.ToString();
            lblVerificadas.Text = totalVerificadas.ToString();
            lblAprobadas.Text = totalAprobadas.ToString();
            lblRechazadas.Text = totalRechazadas.ToString();
            My.MyProject.Forms.Cargando.Close();
