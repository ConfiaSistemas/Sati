...
namespace ConfiaAdmin
{
    ...
    public partial class DatosSolicitudReenviarCorreo
    {
        ...
        public DatosSolicitudReenviarCorreo()
        {
            InitializeComponent();
        }
                ...
                foreach (DataGridViewRow row in dtdatos.Rows)
                {
                    ...
                    foreach (DataRow rowDocumento in dataDocumentosCorreo.Rows)
                    {
                        if (Conversions.ToBoolean(Operators.ConditionalCompareObjectEqual(rowDocumento["IdDatosSolicitud"].ToString(), row.Cells[0].Value, false)))
                        {
                            ...
                            if (File.Exists(Conversions.ToString(Operators.ConcatenateObject(Operators.ConcatenateObject(Operators.ConcatenateObject(Operators.ConcatenateObject(@"C:\ConfiaAdmin\SATI\TempPhotos\", rowDocumento["nombre"]), " "), row.Cells[2].Value), ".jpg"))))
                            {
                                numero += 1;
                                bitmapDocumento.Save(Conversions.ToString(Operators.ConcatenateObject(Operators.ConcatenateObject(Operators.ConcatenateObject(Operators.ConcatenateObject(Operators.ConcatenateObject(Operators.ConcatenateObject(@"C:\ConfiaAdmin\SATI\TempPhotos\", rowDocumento["nombre"]), " "), numero), " "), row.Cells[2].Value), ".jpg")));
                                correo.Attachments.Add(new System.Net.Mail.Attachment(Conversions.ToString(Operators.ConcatenateObject(Operators.ConcatenateObject(Operators.ConcatenateObject(Operators.ConcatenateObject(Operators.ConcatenateObject(Operators.ConcatenateObject(@"C:\ConfiaAdmin\SATI\TempPhotos\", rowDocumento["nombre"]), " "), numero), " "), row.Cells[2].Value), ".jpg"))));
                            }
        ...
        private void BackgroundCorreo_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            ...
            if (errorCorreo)
            {
                MessageBox.Show("El correo no pudo ser enviado satisfactoriamente, favor de intentarlo de nuevo", "SATI", MessageBoxButtons.OK, MessageBoxIcon.Error);
